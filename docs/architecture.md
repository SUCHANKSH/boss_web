...
Phase 5.3 records each committed tracked checkout decrement as an `InventoryAdjustment` with reason `ORDER_CHECKOUT` and the created order as its reference. That history write belongs to the same serializable transaction as the inventory change, order, cart conversion, and idempotency operation, so failed checkouts and idempotent replays cannot create duplicate history.
